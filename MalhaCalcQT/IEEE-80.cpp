...
static const vector<structCabo> Cabo{{"Cobre mole", 0.00393, 234, 1.72, 3.4},
...
void ieee80::resAparente(const Dados &data, double &alpha, double &beta, double &rhoa)
{
    double r;
    r     = sqrt((data.larguraMetros * data.comprimentoMetros) / PI);
...
{
    return ((1000 + 1.5 * fatorCorrecaoCs(data) * Solo[data.solo].resistividade) *
            (0.116 / sqrt(data.tempoDeCurtoSegundos)));
}
...
double ieee80::secaoCabo(const Dados &data)
{
    ...
    T1 = (Cabo[data.cabo].K0 + data.tempMaxMalha) / (Cabo[data.cabo].K0 + data.tempAmbienteCelsius);
...
double ieee80::tensaoToqueMalha(const Dados &data)
{
    ...
    temp = (data.rho2Ohm * data.correnteMalhaAmperes * fatorKm(data) * fatorKi(data)) /
...
double ieee80::tensaoPassoMalha(const Dados &data)
{
    ...
    temp = (data.rho2Ohm * data.correnteMalhaAmperes * fatorKs(data) * fatorKi(data)) /
           compTotalCondutores(temp2);
...
double ieee80::fatorKs(const Dados &data)
{
    ...
    p1 = (1 / PI) * (1.0 / (2 * data.alturaMalhaMetros));
...
{
    return (2 * sqrt(secaoCabo(data) / PI) * 0.001);  // metros
}
